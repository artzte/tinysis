...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    cal = %(<table class="#{options[:table_class]}" width="100%" border="0" cellspacing="0" cellpadding="0">) 
    if options[:pager]
      vars = options[:pager][:data] ? ",#{options[:pager][:data].collect{|a| a.nil? ? 'null' : a}.join(',')}" : ''
      cal << %(<thead><tr class="#{options[:month_name_class]}"><th>)
      cal << link_to_function('&nbsp;', "#{options[:pager][:function]}(#{options[:year]},#{options[:month]},-1#{vars})", :class=>options[:pager][:classes][0])
      cal << %(<th colspan="5">#{Date::MONTHNAMES[options[:month]]} #{options[:year]}</th><th>)
      cal << link_to_function('&nbsp;', "#{options[:pager][:function]}(#{options[:year]},#{options[:month]},1#{vars})", :class=>options[:pager][:classes][1])
